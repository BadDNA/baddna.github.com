...
# the msat id program developed by 'chen'.
# ========================================
...
def create_table(genome):
	...
	query = """DROP TABLE IF EXISTS """ + genome.upper() + """_MSATS_CHEN"""
	
	cursor.execute(query)
	conn.commit() # required for inserting data!!
	
	query =  """CREATE TABLE """ + genome.upper() + """_MSATS_CHEN (
...
def all_msat_parse_infile(file_name,path,genome):
	...
	for line in infile:
		...
		query = """insert into """ + genome.upper() + """_MSATS_CHEN (organism, fasta_id, start_loc, stop_loc, type, repeat_seq, repeat_number) VALUES (%s,%s,%s,%s,%s,%s,%s)"""
